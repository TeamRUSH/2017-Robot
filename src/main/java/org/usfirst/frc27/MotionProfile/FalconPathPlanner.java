...
public class FalconPathPlanner
{
	...
	//talon velocity
	public double[][] talonLeftVelocity;
	public double[][] talonRightVelocity;
 ...
 */
	...
	public void calculate(double totalTime, double timeStep, double robotTrackWidth)
	{
		...
		//get output in a format that the Talon SRX Motion Profile accepts
		talonLeftVelocity = talonVelocity(smoothLeftVelocity, .010, 4.0);
		talonRightVelocity = talonVelocity(smoothRightVelocity, .010, 4.0);	
	}
	
	
	private double[][] talonVelocity(double[][] smoothVelocity, double timeStep, double wheelDiameter) {
		//wheelDiameter should be in inches
		//timeStep should be in seconds
		//
		// smoothVelocity[0] is elapsed time
		// smoothVelocity[1] is velocity in ft/sec
		//
		// To feed to talon, the structure must be:
		// talonVelocity[0] = total revolutions
		// talonVelocity[1] = velocity in rev/sec
		// talonVelocity[2] = time, but not elapsed.  It's the time per step and will generally 
		//                    stay the same (e.g. 10 for 10 milliseconds)
		
		double[][] tmpVelocity = new double [smoothVelocity.length][3];
		
		//The Talon takes timeStep in milliseconds
		double timeStepMillis = timeStep * 1000;
		
		double distPerRev = wheelDiameter * Math.PI;  // 2 * pi * radius = pi * diameter
		
		// Original units are ft/sec.  Need to convert to rpm.
		// ft/sec * 12 in./ft * 1 rev/wheelDiameter*pi inches * 60sec/1minute = rev/minute
		double velocityConversion = 720.0 / distPerRev;
		
		double totalRev = 0;		
		for (int i = 0; i < smoothVelocity.length; i++)
		{
			tmpVelocity[i][1] = smoothVelocity[i][1] * velocityConversion;
			tmpVelocity[i][2] = timeStepMillis;
			if (i == 0)
			{
				tmpVelocity[i][0] = 0;
			}
			...
			{
				double velocityAverage = (smoothVelocity[i][1] + smoothVelocity[i-1][1])/2;
				totalRev += velocityAverage * timeStep;
				tmpVelocity[i][0] = totalRev;
			}
		}
		return tmpVelocity;
	}
