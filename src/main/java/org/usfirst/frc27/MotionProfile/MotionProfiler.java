...
import org.usfirst.frc27.Instrumentation.Instrumentation;
...
public class MotionProfiler {
	...
	// This MotionProfiler class is used by both sides of the robot.
	// _position keeps track of which side of the robot this instance of the
	// motion profiler is tied to.
	// Note: the intention is to remove the need for position.  Eventually,
	//   the points will be passed in from the Command that's running
	//   the profile.
	 ...
	 * @return 
	 */
	public  MotionProfiler(CANTalon talon, String position) {
		_talon = talon;
		_position = position;
		/*
	...
	public void setPath(double[][] newPath) {
		path = newPath;
	}
	...
	private void startFilling() {
		//if (_position == "Left")
		//{
		//	startFilling(LeftMotionProfile.Points, LeftMotionProfile.kNumPoints);
		//}
		//if (_position == "Right")
		//{
		//	startFilling(RightMotionProfile.Points, RightMotionProfile.kNumPoints);
		//}
		System.out.format("%s\n", "start filling 2");
		System.out.println(String.format("%d,", 
				path.length));

		startFilling(path, path.length);
	}
	...
	CANTalon.MotionProfileStatus getStatus(){
		return _status;
	}
