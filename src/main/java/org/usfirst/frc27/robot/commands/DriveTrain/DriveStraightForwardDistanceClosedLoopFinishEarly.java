 ...
 */
public class DriveStraightForwardDistanceClosedLoopFinishEarly extends Command {
	...
	public DriveStraightForwardDistanceClosedLoopFinishEarly(double targetDistance, double targetFeetPerSecond, double errorMultiplier) {
	...
	public DriveStraightForwardDistanceClosedLoopFinishEarly(double targetDistance, double targetFeetPerSecond, double errorMultiplier, double rotationMultiplier) {
		this.targetDistance = targetDistance;
		this.targetFeetPerSecond = targetFeetPerSecond;
		this.errorMultipler = errorMultiplier;
	...
	protected void execute() {
		...
		if(!preFinish){
			Robot.driveTrain.setSpeedSetpointWithOffset(getRPMForFeetPerSecond(targetFeetPerSecond) * -1, rotation);
		} else {
		...
		if((leftData[0] - startDistance[0] > targetDistance || rightData[0] - startDistance[1] > targetDistance)){
